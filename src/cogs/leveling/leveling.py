...
announcement_embed = discord.Embed(
    ...
    description=f"Congrats {LevelUpAnnouncement.Member.mention}! You are now level {LevelUpAnnouncement.LEVEL} 😎",
)
announcement_embed.set_author(name=LevelUpAnnouncement.Member.name)

lvl = DiscordLevelingSystem(
    rate=1, per=10.0, level_up_announcement=LevelUpAnnouncement(announcement_embed)
)
...
class Leveling(commands.Cog):
    ...
    async def givexp(self, ctx, member: discord.Member, amount: int):
        ...
        await lvl.add_xp(member=member, amount=amount)
        await ctx.send(
            f"Gave {amount} xp to {member.name}, Removed {amount} xp from {ctx.author.name}"
        )

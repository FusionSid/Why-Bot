  ...
  async def help(self, ctx, cat=None, cmd=None):
    ...
    if cat == None:
      ...
      if cat.lower() == 'fun':
        ...
        embed.add_field(name="`?dm <@person> <message>`",value="The bot dms someone")
        embed.add_field(name="`?yesorno <questions>`",value="Creates a yes or no poll")
        embed.add_field(name='`?poll <time:seconds> "<question>" <options seperated by a space>`',value='Creates a poll with multiple options and shows result when time is finished. Polls longer than 1h are not guaranteed to work')

...
class Database(commands.Cog):
  ...
  async def store(self, ctx):
    ...
    with conn:
        c.execute(f"INSERT INTO {ctx.author.name} (name, password) VALUES (:name, :password)", {'name':name, 'password':evalue})
    await ctx.send("Done")
    os.chdir(cd)
  ...
  async def get(self, ctx):
    ...
    def find(search_type="all", search=None):
      if search_type == 'name':
          with conn:
              c.execute(f"SELECT * FROM {ctx.author.name} WHERE name = :name", {'name': search})
          return c.fetchall()

      if search_type == 'id':
          with conn:
              c.execute(f"SELECT * FROM {ctx.author.name} WHERE id = :id", {'id': int(search)})
          return c.fetchall()
